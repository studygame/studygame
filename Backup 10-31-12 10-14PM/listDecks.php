...
<body>
	...
	<div>
		<form action="listDecks.php" method="POST">
			...
			<input type="submit" value="Create!" name="createdeck">	
		</form>
	</div>
<?php
...
?>
    <form action="listDecks.php" method="POST">
		<input type="submit" name="Home" value="Return to Lobby">
		<?php
			if(isset($_POST['Home'])) {
				?>
...
function deckTable($row){
	...
	echo '<td> <input class="even" type="submit" name="updatedeck" value="Edit Cards?"  /></td>';
	echo '</form>';
	echo '<form action="listDecks.php" method="POST">';
	echo '<input type="hidden" name="deckid" value="'.$row['deckid'].'"/>';
	echo '<td> <input class="odd" type ="submit" name="deletedeck" value="Delete?" /></td>';
	echo '</tr>';
	echo '</form>';
}
?>
