	...
	switch($_POST["function"]) {
		...
		case("joinGame"):
			...
			// if (empty($curGameState)) {
				...
				$curGameState["startTimer"] = $startTimer;	// seconds before a game starts
		...
		case("getGameState"):
			...
			if (($lock == 0 && $timediff >=1 && $playerID == $nextUpdatePlayerID) || ($lock == 0 && $timediff >=1)) {
				...
				if (pg_affected_rows($result) > 0) {
					...
					if ($curGameState["state"] == 0) {
						
						// set player gamescore to zero
						$curGameState["players"][$playerID]["gamescore"] = 0;

						$curGameState["timestamp"] = $now;
						$curGameState["startTimer"]--;
				
						// if start counter has ended, start the game
						if ($curGameState["startTimer"] == -1) {

							// initialize game data to start a new game
							$curGameState["startTimer"] = 0;
					...
					// if game is in progress, check for game state changes
						...
						$curGameState["timestamp"] = $now;
						$curGameState["cardTimer"]--;
		
						// if time has run out for the current card
						if ($curGameState["cardTimer"] == -1) {
					...
					// if game is between rounds, wait a few seconds
						...
						$curGameState["timestamp"] = $now;
						$curGameState["roundTimer"]--;
				
						// when timer runs out, continue to the next round
						if($curGameState["roundTimer"] == -1) {
					...
					// if game is over, wait a few seconds
						...
						$curGameState["timestamp"] = $now;
						$curGameState["endTimer"]--;
				
						// when timer runs out, start a new game
						if($curGameState["endTimer"] == -1) {
